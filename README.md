...
Markets use TradingView's official keyless Ticker Tape web component by default. Alpha Vantage remains an optional advanced-provider prototype in Settings and never blocks the default market display. A personal Alpha Vantage key is stored in browser local storage and is visible to browser scripts and developer tools, so do not use a privileged or developer-owned secret.

The application does not use OpenWeather. Currents credentials are never frontend configuration.

### TradingView and content security policy

The market panel loads one official ES-module web component from `https://widgets.tradingview-widget.com`. Deployments with a Content Security Policy must allow that origin in `script-src`/`script-src-elem` and allow TradingView HTTPS and WebSocket endpoints in the relevant `connect-src`, `img-src`, and (if requested by a future widget version) `frame-src` directives. Keep the policy scoped to `https://*.tradingview.com`, `https://*.tradingview-widget.com`, and `wss://*.tradingview.com`, then confirm the exact requests with CSP violation reporting on the production origin. If the script is blocked, Ambient Brief keeps a stable placeholder before showing an explicit unavailable state.
...
- TradingView markets are keyless; Alpha Vantage is optional and does not block startup.
