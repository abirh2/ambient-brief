...
The update workflow uses at most five Currents requests per run. It fetches a broad English feed plus focused US, business, sport, and entertainment feeds, validates the provider responses with Zod, normalizes and conservatively deduplicates the articles, ranks each application category, validates the complete generated document, and atomically replaces the previous file. Partial runs retain prior valid data for categories whose owning request failed. If every request fails, the prior file is left untouched and the workflow fails.
